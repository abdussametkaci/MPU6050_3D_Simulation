...
ser = serial.Serial(
    ...
    bytesize=serial.EIGHTBITS,\
        timeout=0.1) # 1 saniyede bir okur
...
def read_values():
    ...
    if line == b'':
        ...
        angles[0] = -1*float(lines[1])
        angles[1] = -1*float(lines[2][:4])
        angles[2] = -1*float(lines[0][2:])
    print(angles[0], ",", angles[1], ",", angles[2])
    return angles # aci cinsinden deger (x, y, x)
